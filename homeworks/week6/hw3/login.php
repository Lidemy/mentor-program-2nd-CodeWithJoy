...
@$acc =$_POST['account']; 
@$pwd = $_POST['password'];
	$sqlName = "SELECT username FROM joy_board_register WHERE account = '$acc' ";
	$resultName = $conn->query($sqlName)->fetch_assoc();
@$name = $resultName['username'];

	$sqlPwd = "SELECT password FROM joy_board_register WHERE account = '$acc' ";
	$resultPwd = $conn->query($sqlPwd)->fetch_assoc();
@$hash= $resultPwd['password']; 

/* 把session_id 存入db */
$session_id = uniqid(); //創造session_id
$sql2 = "INSERT INTO `joy_users_certificate` (`session_id`, `account`) VALUES ('$session_id', '$acc')";

$sql = "SELECT * FROM joy_board_register WHERE account = '$acc' ";
