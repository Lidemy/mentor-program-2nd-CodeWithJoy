        ...
        <div class="main__content__wrap">
            ...
            <div> <?echo htmlspecialchars($row['content'], ENT_QUOTES, 'utf-8') ?>
              ...
              ?>
                  <form class="editForm" action="edit.php" class="edit" method="POST">
                    <input class="btn btn-primary edit editImg " type="button"  value="edit"   > 		
                  ...
                  <form  class="deleteForm"  method="POST">
                    <input type="button" class="delete deleteImg btn btn-danger" value="delete"  > 		
            ...
            <!-- 子留言 -->
                ...
                ?>
                  <div class="wrap"> 
                    ...
                    <input  type="button" class="btn btn-primary edit editImg" value='edit' > 		
                      <input type="hidden" name="article_id" value=<? echo htmlspecialchars($row2['id'], ENT_QUOTES, 'utf-8'); ?> >
                    </form>
                    <form action="delete.php" method="POST">
                      <input type="button" class="btn btn-danger delete deleteImg" value="delete"> 
                      <input type="hidden" name="article_id" value=<? echo htmlspecialchars($row2['id'], ENT_QUOTES, 'utf-8'); ?> >
                    </form>
                </div>
              </div>
            ...
            <div class="child__form">
              <form class="child__form" action="add_content_child.php" method='post'>
                ...
                <input class="btn btn-info child__submit" type='submit' value='留言'>
              </form>
            </div>
  ...
  function goBack(){
   ...
   //ajax request
      $(document).ready(function(){
        $("form[class=user__msg]").on("submit",function(e){
          e.preventDefault();           
                    ...
                    <div class="main__content__wrap">
                      <div class='wrap  space-between'>
                        ...
                        <div class="wrap" style = "display:flex">
                            <form class="editForm" action="edit.php" class="edit" method="POST">
                              <input class="btn btn-primary edit editImg " type="button"  value="edit"   > 		
                              <input class="artile__id__hidden" type="hidden" name="article_id" value=${res.id} >
                              <input type="hidden" name="hash_id" value="1" ><!--  確認是來自主留言，可編輯hash -->
                            </form>
                            <form  class="deleteForm"  method="POST">
                              <input type="button" class="delete deleteImg btn btn-danger" value="delete"  > 		
                              <input class="artile__id__hidden2" type="hidden" name="article_id" value=${res.id} >
                            </form>
                            <br>
                        </div>
                          <div class="child__form">
        ...
        $(".editImg").on("click",function(e){
          e.preventDefault();
          var articleId = e.target.nextElementSibling.value; 
          ...
          content.outerHTML = `<textarea class='temp'>`+content_txt+`</textarea><br><input type='button' class='newTxt btn btn-warning' value='送出'> <input type='button' class='goBack btn btn-secondary' value='返回' onclick="goBack()">`
          
          //var new_hashtags = e.target.parentNode.parentNode.parentNode.previousSibling.previousSibling;
          
          $(".newTxt").on("click",function(){
            e.preventDefault();
            ...
            $.ajax({
              ...
              success: function(resp){
                  ...
                  if(res.result ==='success' ){
                    ...
                    `);
                    $("input[class='newTxt btn btn-warning']").remove();
                    $("input[class='goBack btn btn-secondary']").remove();
                    }
                }
            })
          })
